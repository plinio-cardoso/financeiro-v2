<tr wire:key="transaction-{{ $transaction->id }}" class="hover:bg-gray-100 dark:hover:bg-gray-700/30 transition-colors">
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <div x-data="{
            ...
            <div x-show="!editing" @click="editing = true" class="flex items-center gap-2 group cursor-pointer">
                ...
                @endif
                <span class="text-sm font-bold text-gray-900 dark:text-gray-100 group-hover:text-[#4ECDC4] transition-colors" x-text="value"></span>
            ...
            <input x-show="editing" x-cloak x-ref="input" x-model="value"
                @focusout="save()" @keydown.enter="save()" @keydown.escape="editing = false; value = original"
        ...
        <div @click="$wire.edit()" class="mt-1 group cursor-pointer min-h-[1.25rem]">
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <div x-data="{
            ...
            <div x-show="!editing" @click="editing = true" class="flex items-center gap-2 group cursor-pointer">
                <span class="text-sm font-bold text-gray-900 dark:text-gray-100 group-hover:text-[#4ECDC4] transition-colors" x-text="formatDisplay()"></span>
                <svg class="w-3 h-3 text-[#4ECDC4] opacity-0 group-hover:opacity-100 transition-opacity"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                        d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <div @click="$wire.edit()" class="cursor-pointer inline-block">
            <span @class([
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        <div x-data="{
            ...
            <div x-show="!editing" @click="editing = true" class="flex items-center gap-2 group cursor-pointer">
                <span class="text-sm font-medium text-gray-900 dark:text-gray-100 group-hover:text-[#4ECDC4] transition-colors" x-text="formatDisplay()"></span>
                <svg class="w-3 h-3 text-[#4ECDC4] opacity-0 group-hover:opacity-100 transition-opacity"
                    fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                        d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z" />
            ...
            <input x-show="editing" x-cloak x-ref="input" x-model="value"
                @focusout="save()" @change="save()" @keydown.enter="save()" @keydown.escape="editing = false; value = original"
    ...
    <td class="px-6 py-4 whitespace-nowrap">
        @if($transaction->tags->isNotEmpty())
            <div @click="$wire.edit()" class="flex flex-wrap gap-1 cursor-pointer p-1">
                @foreach($transaction->tags as $tag)
                    <span class="inline-flex items-center px-2 py-0.5 text-[10px] font-black rounded-md uppercase tracking-wider transition-opacity hover:opacity-80"
        ...
        @else
            <div @click="$wire.edit()" class="cursor-pointer p-1">
                <span class="text-xs text-gray-400 dark:text-gray-500 italic hover:text-[#4ECDC4] transition-colors">Sem tags</span>
            </div>
        @endif
    </td>
