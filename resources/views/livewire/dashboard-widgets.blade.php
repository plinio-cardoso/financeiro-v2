<div class="space-y-8 pb-12">
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        <div class="overflow-hidden bg-white shadow sm:rounded-lg dark:bg-gray-800 flex flex-col"
            ...
            <div class="px-8 py-6 border-b border-gray-50 dark:border-gray-700/30 flex items-center justify-between">
                <h3 class="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Próximas
                    Despesas</h3>
                <a href="{{ route('transactions.index', ['filterStatus' => 'pending', 'filterType' => 'debit', 'sortField' => 'due_date', 'sortDirection' => 'asc']) }}"
                ...
                <div class="space-y-8">
                    @forelse($this->upcomingExpensesGrouped as $timestamp => $dayExpenses)
                        ...
                        <div class="space-y-4">
                            <h4 class="text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400 px-1">
                                @if($date->isToday()) HOJE @elseif($date->isTomorrow()) AMANHÃ @else
                                {{ $date->translatedFormat('D, d M') }} @endif
                            </h4>
                    ...
                    @empty
                        <div class="py-12 text-center">
                            <p class="text-xs font-black uppercase tracking-widest text-gray-500 dark:text-gray-400">Tudo em
                                dia!</p>
                        </div>
                    @endforelse
                </div>
        ...
        {{-- Histórico Recente --}}
        <div class="overflow-hidden bg-white shadow sm:rounded-lg dark:bg-gray-800 flex flex-col"
            x-intersect.once="$wire.loadRecentActivity()">
            <div class="px-8 py-6 border-b border-gray-50 dark:border-gray-700/30 flex items-center justify-between">
                <h3 class="text-xs font-black uppercase tracking-widest text-gray-400 dark:text-gray-500">Atividades
                    Recentes</h3>
                <span class="text-[10px] font-black uppercase tracking-widest text-gray-900 dark:text-gray-100">Últimas
                    5</span>
            </div>
                ...
                <div class="space-y-1">
                    @foreach($recentActivity as $activity)
                        <div
                            ...
                            <div class="flex items-center gap-4">
                                <div
                                    class="w-10 h-10 rounded-lg bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-700 flex items-center justify-center">
                                    <svg class="w-5 h-5 {{ $activity->getTypeColorClass() }}" fill="none"
                                        viewBox="0 0 24 24" stroke="currentColor">
                                ...
                                <div>
                                    ...
                                    <p
                                        class="text-[10px] text-gray-500 dark:text-gray-400 uppercase tracking-widest font-black mt-0.5">
                            ...
                            <div class="text-right">
                                ...
                                @if($activity->status->value === 'paid')
                                    <span
                                        class="text-[8px] font-black uppercase tracking-widest text-emerald-600 px-1.5 py-0.5 bg-emerald-50 dark:bg-emerald-500/10 rounded-md">Pago</span>
                                @else
                                    <span
                                        class="text-[8px] font-black uppercase tracking-widest text-yellow-700 px-1.5 py-0.5 bg-yellow-50 dark:bg-yellow-600/10 rounded-md">Pendente</span>
                                @endif
                            </div>
                        </div>
                    @endforeach
