                ...
                <div class="p-6">
                    ...
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $transaction->getFormattedAmount() }}</dd>
                        </div>

                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Tipo</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $transaction->isDebit() ? 'bg-red-100 text-red-800' : 'bg-green-100 text-green-800' }}">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">
                                <span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full {{ $transaction->isPaid() ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800' }}">
                        ...
                        <div>
                            ...
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $transaction->getFormattedDueDate() }}</dd>
                        </div>

                        @if($transaction->paid_at)
                        <div>
                            <dt class="text-sm font-medium text-gray-500 dark:text-gray-400">Data de Pagamento</dt>
                            <dd class="mt-1 text-sm text-gray-900 dark:text-gray-100">{{ $transaction->paid_at->format('d/m/Y') }}</dd>
                        </div>
                        @endif
                            ...
                            <dd class="mt-1">
                                <div class="flex flex-wrap gap-2">
                                    @foreach($transaction->tags as $tag)
                                        <span class="px-2 py-1 text-xs rounded-full" style="background-color: {{ $tag->getColorWithDefault() }}20; color: {{ $tag->getColorWithDefault() }};">
                                            {{ $tag->name }}
                                        </span>
                                    @endforeach
                                </div>
